...
describe("SellerIntakeForm", () => {
  ...
  test("submits normalized seller payload and includes Nexus mandatory ingestion fields", async () => {
  ...
  test("fails visibly when VITE_NEXUS_ORG_ID is missing", async () => {
    ...
    const errorEl = await screen.findByTestId("seller-error");
    expect(errorEl.textContent).toContain("NEXUS_ORG_ID is missing");
  });

  test("displays human-readable message for structured 422 errors stripping 'body.' prefix", async () => {
    fetchMock.mockResolvedValueOnce({
      ok: false,
      json: async () => ({
        detail: [
          { loc: ["body", "email"], msg: "invalid email format", type: "value_error.email" },
          { loc: ["body", "org_id"], msg: "Field required", type: "value_error.missing" }
        ]
      }),
    });
    ...
    expect(errorEl.textContent).toContain("org_id: Field required");
